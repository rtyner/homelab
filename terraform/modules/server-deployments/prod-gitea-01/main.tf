resource "proxmox_vm_qemu" "master-deployment" {
    ...
    provisioner "local-exec" {
        command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${each.value.ip_address}, /Users/rt/repos/local.rtyner.com/ansible/playbooks/infrastructure/enable-unattended-upgrades.yml"
    }   

    provisioner "local-exec" {
        command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${each.value.ip_address}, /Users/rt/repos/local.rtyner.com/ansible/playbooks/infrastructure/resolv-dns-config.yml"
    }               
}
