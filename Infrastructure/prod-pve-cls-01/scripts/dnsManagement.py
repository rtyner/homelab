...
class DNSManager:
    def __init__(self, host='prod-bind-01', user='rt'):
        self.ssh = paramiko.SSHClient()
        self.ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        self.ssh.connect(host, username=user)
        
        self.forward_zone = "/etc/bind/zones/db.local.rtyner.com"
        self.reverse_zone = "/etc/bind/zones/db.10.1.1"
        self.domain = "local.rtyner.com"
    ...
    def backup_zone_files(self):
        ...
        try:
            ...
            # Backup forward zone
            forward_backup = f"/etc/bind/zones/backups/db.local.rtyner.com.{timestamp}"
    ...
    def check_record_exists(self, hostname, record_type='A'):
        try:
            ...
            hostname_full = f"{hostname}.{self.domain}"
            
            # Check each line for the record
            for line in content.split('\n'):
                if line.strip() and not line.strip().startswith(';'):
                    if hostname_full in line and f"IN    {record_type}" in line:
                        return True
            return False
        except Exception as e:
    ...
    def add_dns_record(self, hostname, ip, record_type='A'):
        ...
        if self.check_record_exists(hostname, record_type):
            print(f"Record already exists for {hostname}.{self.domain}")
            return False
        ...
        try:
            ...
            # Reload BIND
            self.execute_command("sudo rndc reload")
            print(f"Successfully added {record_type} record for {hostname}.{self.domain}")
            return True
